...
static void XN297L_GPIO_Config(void){
  GPIOC -> DDR = (1<<3)|(1<<5)|(1<<6);
  GPIOC -> CR1 = (1<<3)|(1<<4); 
  GPIOC -> CR2 = (1<<5)|(1<<6);
  
  GPIOD -> DDR = (1<<2)|(1<<3);
  GPIOD -> CR1 = (1<<2)|(1<<3); 	// RXD上拉输入
  GPIOD -> CR2 = 0x00; 				// TXD高速输出
  GPIOD -> ODR |= (1<<2);   
}
...
** 写入寄存器数值
*/
static void rfWriteReg( u8 ucRegAddr,  u8 ucData){
    spiCSOn();
...
** 读取寄存器数值
*/
static u8 rfReadReg(u8 ucRegAddr){
    u8 ucData;  
    spiCSOn();
    spiLoopSendReceive(ucRegAddr);
...
** 写入寄存器N个数值
*/
static void rfWriteNRegister(u8 ucRegAddr, u8* pusData, u8 ucCnt){
    u8 i; 
    spiCSOn();
    spiLoopSendReceive(ucRegAddr);
    for (i = 0; i < ucCnt; i++) {
...
u8 rfSendNData(u8 *ucData, u8 ucLength){
    ...
    delay1us(10);                                                             //keep ce high at least 16us
    rfCE_Low();                                                               //rf entery stb3
